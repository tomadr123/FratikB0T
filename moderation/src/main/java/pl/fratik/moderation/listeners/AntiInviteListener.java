...
public class AntiInviteListener {
    ...
    private static final Cache<String, Kara> antiinviteKaraCache = Caffeine.newBuilder().maximumSize(1000).expireAfterWrite(10, TimeUnit.MINUTES).build();
    ...
    public void onMessage(MessageReceivedEvent e) {
        ...
        if (content.contains("discord.gg/") || content.contains("discord.io/") || content.contains("discord.me/") ||
            ...
            try {
                ...
                Kara kara = getKara(e.getGuild());
                Language language = tlumaczenia.getLanguage(e.getMember());
                if (kara == null) {
                    e.getChannel().sendMessage(tlumaczenia.get(language,
                            "antiinvite.notice", e.getAuthor().getAsMention(),
                            managerKomend.getPrefixes(e.getGuild()).get(0))).queue();
                    return;
                }
                synchronized (e.getGuild()) {
                    Case c = new CaseBuilder(e.getGuild()).setUser(e.getAuthor().getId()).setKara(kara)
                    ...
                    switch (kara) {
                        case BAN:
                            e.getChannel().sendMessage(tlumaczenia.get(language,
                                    "antiinvite.notice.ban", e.getAuthor().getAsMention(),
                                    managerKomend.getPrefixes(e.getGuild()).get(0))).queue();
                            break;
                        case KICK:
                            e.getChannel().sendMessage(tlumaczenia.get(language,
                                    "antiinvite.notice.kick", e.getAuthor().getAsMention(),
                                    managerKomend.getPrefixes(e.getGuild()).get(0))).queue();
                            break;
                        case MUTE:
                            e.getChannel().sendMessage(tlumaczenia.get(language,
                                    "antiinvite.notice.mute", e.getAuthor().getAsMention(),
                                    managerKomend.getPrefixes(e.getGuild()).get(0))).queue();
                            break;
                        case WARN:
                            e.getChannel().sendMessage(tlumaczenia.get(language,
                                    "antiinvite.notice.warn", e.getAuthor().getAsMention(),
                                    managerKomend.getPrefixes(e.getGuild()).get(0))).queue();
                            break;
                        default:
                            LoggerFactory.getLogger(getClass()).warn("nieprawidłowa kara");
                            e.getChannel().sendMessage(tlumaczenia.get(language,
                                    "antiinvite.notice.warn", e.getAuthor().getAsMention(),
                                    managerKomend.getPrefixes(e.getGuild()).get(0))).queue();
                    }
                    ...
                    cr.getCases().add(c);
                    casesDao.save(cr);
                    if (kara == Kara.WARN) WarnUtil.takeAction(guildDao, casesDao, e.getMember(), e.getChannel(),
    ...
    private Kara getKara(Guild guild) {
        return antiinviteKaraCache.get(guild.getId(), id -> guildDao.get(guild).getKaraZaReklame());
    }
    ...
    public void onDatabaseUpdate(DatabaseUpdateEvent e) {
        ...
        antiinviteIgnoreChannelsCache.invalidate(((GuildConfig) e.getEntity()).getGuildId());
        antiinviteKaraCache.invalidate(((GuildConfig) e.getEntity()).getGuildId());
    }
}
